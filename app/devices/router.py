...
@router.post(
    "", response_model=DeviceRegisterResponse, status_code=status.HTTP_201_CREATED
)
async def register_new_device(
    ...
    return await device_service.create_device(db, device, user_id)


@router.get("", response_model=list[DeviceListResponse], status_code=status.HTTP_200_OK)
async def list_registered_devices(
    pagination: PaginationParams = Depends(),
    filter: DeviceFilter = Depends(),
    db: AsyncSession = Depends(get_db),
    user_id: UUID = Depends(get_authenticated_user_id),
):
    """Retrieves the full fleet of devices owned by the user."""
    return await device_service.list_devices_by_user_id(db,pagination, filter, user_id)


@router.patch(
    "/revoke-all", response_model=MessageResponse, status_code=status.HTTP_200_OK
)
...
async def get_device_details(
    ...
    return await device_service.get_device_by_id(db, device_id, user_id)


@router.patch(
    "/{device_id}/status",
    response_model=DeviceToggleResponse,
...
async def toggle_device_power_switch(
    ...
    return await device_service.toggle_device_by_id(db, device_id, user_id)
...
async def rotate_device_secret(
    ...
    return await device_service.rotate_key_by_id(db, device_id, user_id)
...
@router.delete(
    "/{device_id}", response_model=MessageResponse, status_code=status.HTTP_200_OK
)
async def decommission_device(
    ...
    return await device_service.delete_device_by_id(db, device_id, user_id)
